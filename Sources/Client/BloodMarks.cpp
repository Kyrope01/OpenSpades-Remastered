...
DEFINE_SPADES_SETTING(cg_bloodMarks, "1");
...
namespace spades {
	namespace client {
		namespace {
			...
			bool IsBloodMarksEnabled() { return cg_blood && cg_bloodMarks; }

			Vector3 RandomBloodVector() {
				return MakeVector3(SampleRandomFloat() - SampleRandomFloat(),
		...
		Assets::Assets() {
			...
			// Floor and ceiling
			orientations.at(4) = Matrix4::Rotate(Vector3{1, 0, 0}, -(float)M_PI * 0.5F) *
			                     Matrix4::Translate(0.0F, -0.2F, 0.0F);
			orientations.at(8) = Matrix4::Rotate(Vector3{1, 0, 0}, (float)M_PI * 0.5F) *
			                     Matrix4::Translate(0.0F, -0.2F, 0.0F);
				...
				for (size_t k = 1; k < 4; ++k) {
					orientations.at(i + k) =
					  Matrix4::Rotate(Vector3{0, 0, 1},
						  (float)M_PI * 0.5F * float(k)) * orientations.at(i);
				}
			}
		...
		BloodMarks::BloodMarks(Client& client) : impl{new BloodMarksImpl{client}} {
			...
			std::transform(assets.models.begin(), assets.models.end(),
			               std::back_inserter(impl.models), [&](Handle<VoxelModel> model) {
				               return Handle<IModel>(client.GetRenderer()->CreateModel(model), false);
			               });
		...
		void BloodMarks::Spatter(const Vector3& position, const Vector3& velocity, bool byLocal) {
			...
			for (const float& e : energyDistribution) {
				for (int attempt = 0; attempt < 3; ++attempt) {
					...
					particleVelocity *= e;
					particleVelocity += RandomBloodVector() * (velocity.GetLength() * 0.2F);
